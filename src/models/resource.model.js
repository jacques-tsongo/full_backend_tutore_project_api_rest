...
exports.create = async (name, data, extra = {}) => { const d = schema[name]; const fields = d.fields.filter((f) => data[f] !== undefined); if (extra.ownerField) { fields.push(extra.ownerField); data[extra.ownerField] = extra.ownerId; } const [r] = await db.execute(`INSERT INTO ${d.table} (${fields.join(', ')}) VALUES (${fields.map(() => '?').join(', ')})`, fields.map((f) => data[f])); return exports.get(name, r.insertId); };
exports.update = async (name, id, data) => { const d = schema[name]; const fields = d.fields.filter((f) => data[f] !== undefined); if (!fields.length) return exports.get(name, id); await db.execute(`UPDATE ${d.table} SET ${fields.map((f) => `${f} = ?`).join(', ')} WHERE ${d.id} = ?`, [...fields.map((f) => data[f]), id]); return exports.get(name, id); };
