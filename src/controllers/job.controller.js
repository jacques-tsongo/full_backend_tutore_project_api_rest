...
exports.myApplications = asyncHandler(async (req, res) => { 
    const [rows] = await db.execute('SELECT c.*, o.titre_offre, o.localisation, e.nom_entreprise, m.score_compatibilite FROM candidature c JOIN offre_emploi o ON o.id_offre=c.id_offre JOIN entreprise e ON e.id_entreprise=o.id_entreprise LEFT JOIN matching m ON m.id_utilisateur=c.id_utilisateur AND m.id_offre=c.id_offre WHERE c.id_utilisateur=? ORDER BY c.date_candidature DESC', [req.user.id_utilisateur]); 
        success(res, 'Mes candidatures.', { items: rows }); });
exports.cancel = asyncHandler(async (req, res) => { 
    const [r] = await db.execute("UPDATE candidature SET statut_candidature='Annulée' WHERE id_candidature=? AND id_utilisateur=? AND statut_candidature='En attente'", [req.params.id, req.user.id_utilisateur]); 
...
exports.updateApplicationStatus = asyncHandler(async (req, res) => { 
    ...
    await notify.create(rows[0].id_utilisateur, `Votre candidature pour « ${rows[0].titre_offre} » est maintenant : ${req.body.statut_candidature}.`); 
    success(res, 'Statut de candidature mis à jour.'); });

    
exports.matchOffer = asyncHandler(async (req, res) => success(res, 'Score de compatibilité calculé.', { matching: await matching.calculate(req.user.id_utilisateur, req.params.id) }));
