...
const companyFor = async (id) => (
    await db.execute('SELECT r.id_entreprise, e.statut_validation FROM recruteur r JOIN entreprise e ON e.id_entreprise=r.id_entreprise WHERE r.id_utilisateur=?', [id]))[0][0];
exports.create = asyncHandler(async (req, res) => { 
    const company=await companyFor(req.user.id_utilisateur); 
    if(!company || company.statut_validation!=='Validée') return fail(res, 'Entreprise validée et profil recruteur requis.', [], 403); 
